...
```{r eval=FALSE, echo=TRUE}
fastqc /*001.fastq.gz -o /FastQC_Reports
```
...
```{r eval=FALSE, echo=TRUE}
for i in $(ls ~/ | sed s/_R1_001.fastq.gz// |sort -u | grep -v '_R2');do echo ${i}; salmon quant -i ~/salmon_index/  --libType A --seqBias -- gcBias  -1 ~/${i}_R1_001.fastq.gz -2 ~/${i}_R2_001.fastq.gz --validateMappings -p 20 -o ~/Salmon_output/${i}; done 
```
...
```{r}
csvfile <- file.path("~/", "study_design_GcgrKO.csv")
coldata <- read.csv(csvfile, stringsAsFactors=FALSE)
coldata$names <- coldata$Sample
coldata$files <- file.path("~/", coldata$names, "quant.sf")

head(coldata,10)
```

The Summarized Experiment object consisting of transcript names and count/abundances plus meta data is created:
```{r warning=FALSE}
se <- tximeta(coldata, type = "salmon")
dim(se)
```

140790 transcripts across the 10 samples are summarized to gene level with *tximeta*: 
```{r message=FALSE, warning=FALSE}
gse <- summarizeToGene(se)
dim(gse)
```
54307 genes across the 10 samples.
...
A DESeq2 object with comparisons made across the *Condition*  is created:
```{r warning=FALSE}
...
Condition <- dds@colData@listData[["Condition"]]
anno <- data.frame(Condition, row.names = dds@colData@rownames)
annotation_colors <- list( 
  Condition = c("GcgrKO" = "#FF3333","WT" = "#000000")) 
...
Sample GR431 seems like an outlier and is removed: 
```{r}
vst_idx <- grep("GR431", colnames(vst)) 
rvst <- vst[,-vst_idx]
```

The EDA plots are created without GR431:
```{r}
pcaData <- plotPCA(rvst, intgroup = c("Condition", "Sample"), returnData = TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
...
ggplot(pcaData, aes(x = PC1, y = PC2, color = Condition, label = Sample)) +
  ...
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed() +
  ggtitle("PCA with vst normalized data - GR431 removed") +
  theme_bw()

sampleDists <- dist(t(assay(rvst)))
sampleDistMatrix <- as.matrix(sampleDists)
rows <- rownames(sampleDistMatrix)
rownames(sampleDistMatrix) <- NULL
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
...
pheatmap(sampleDistMatrix,
         ...
         clustering_method = "complete",
         col = colors,
         main = "Heatmap with hierarchical clustering - GR431 removed",
         height = 10,
         width = 5)
```

Sample GR431 is a clear outlier and is removed: 
```{r}
idx <- grep("GR431", colnames(dds)) 
dds <- dds[,-idx]
dds
```
...
KOvsWT <- results(dds, contrast=c("Condition","GcgrKO","WT"),
...
```{r fig.align='center', message=FALSE, warning=FALSE}
KOvsWT_tibble <- KOvsWT %>%
  as.data.frame(.) %>%
  rownames_to_column("ENSEMBL") %>%
  as_tibble(.)

ggplot(KOvsWT_tibble) +
  ...
  labs(title = "MA-plot (GcgrKO vs WT)",
       y = "Log2 fold change",
...
```{r, fig.align='center'}
hist(KOvsWT$pvalue[KOvsWT$baseMean > 1], breaks = 0:20/20,
     col = "grey50", border = "white", main = "P-value distribution for GcgrKO vs WT",
     xlab = "P-value")
```
...
```{r fig.align='center', message=FALSE, warning=FALSE}
ggplot(KOvsWT_tibble, aes(x=dense_rank(baseMean), y=-log10(pvalue), color=padj < 0.05)) + 
  geom_point(alpha=0.5) + 
  labs(title = "Independent filtering (GcgrKO vs WT)",
       y = "- Log10 p-value",
...
```{r message=FALSE, warning=FALSE}
ens.str <- substr(rownames(KOvsWT), 1, 18)
KOvsWT$symbol <- mapIds(org.Mm.eg.db,
                     keys=ens.str,
                     column="SYMBOL",
                     keytype="ENSEMBL",
                     multiVals="first")
KOvsWT$gene_id <- mapIds(org.Mm.eg.db,
                     keys=ens.str,
                     column="ENTREZID",
                     keytype="ENSEMBL",
                     multiVals="first")
KOvsWT$gene_name <- mapIds(org.Mm.eg.db,
                     keys=ens.str,
                     column="GENENAME",
                     keytype="ENSEMBL",
                     multiVals="first")

KOvsWT_tibble <- add_column(KOvsWT_tibble,gene_symbol = KOvsWT$symbol, .after = "ENSEMBL")
KOvsWT_tibble <- add_column(KOvsWT_tibble,gene_name = KOvsWT$gene_name, .after = "gene_symbol")
```

All differentially expressed genes (FDR < 0.05) are saved as *All_Sig_Genes_KOvsWT.xlsx*
```{r}
All_Sig_Genes_KOvsWT <- arrange(KOvsWT_tibble, desc(log2FoldChange)) %>% 
  filter(., padj < 0.05) %>%
  mutate(., foldChange = 2^log2FoldChange) 

write_xlsx(All_Sig_Genes_KOvsWT, "All_Sig_Genes_KOvsWT.xlsx")
write_csv(All_Sig_Genes_KOvsWT, "All_Sig_Genes_KOvsWT.csv")
```
...
```{r}
GO_KOvsWT <- KOvsWT
genes <- lapply(rownames(GO_KOvsWT), function(x) unlist(strsplit(x,".", fixed=TRUE))[1])
rownames(GO_KOvsWT) <- genes
resSig <- subset(GO_KOvsWT, padj < 0.05)
resFCordered <-unique(GO_KOvsWT[order(GO_KOvsWT$log2FoldChange, decreasing = TRUE),])
sigFCordered <-unique(resSig[order(resSig$log2FoldChange, decreasing = TRUE),])

universe <- resFCordered[,c("log2FoldChange")]

siggenes <- sigFCordered[abs(sigFCordered$log2FoldChange) > 0,c("log2FoldChange")]
names(universe) <- rownames(resFCordered)
names(siggenes) <- rownames(sigFCordered[abs(sigFCordered$log2FoldChange) > 0,])
...
formula_res <- compareCluster(Ensembl~Genes, 
                              data=mydf, 
...
```{r}
export_formula_res <- as_tibble(formula_res) %>% dplyr::select(c(-"Cluster",-"qvalue",-"Count"))

write_xlsx(export_formula_res, "GOBP_Genes_KOvsWT.xlsx")
write_csv(export_formula_res, "GOBP_Genes_KOvsWT.csv")
```

The lists used for making the GOBP analysis is saved:
```{r}
write.csv(mydf, "mydf_GcgrKO.csv")
write.csv(universe, "universe_GcgrKO.csv")
```
...
t_box_shiny <- tibble(count = 1, Condition = NA, ENSEMBL = NA)

for(i in rownames(dds)){
  pCdata_box <- plotCounts(dds, gene = i, intgroup = "Condition", returnData =  TRUE)  %>% as_tibble() %>% add_column(ENSEMBL = i) 
...
```{r eval=FALSE, include=TRUE}
Export_KOvsWT_tibble <- KOvsWT_tibble %>% 
  mutate(., foldChange = 2^log2FoldChange) %>%
  relocate(., foldChange, .after = log2FoldChange)

write_csv(Export_KOvsWT_tibble, file = "~/KO_WT_shiny.csv")
